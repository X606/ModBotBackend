/*----------------------------------------------------------------------------------------------+
|                                                                                               |
|     This code was generated by the ModBotWebistieAPICodeGenerator, do not change it directly  |
|                                                                                               |
| /---------------------------------------------------------------------------------------------+
| |
| | Operation Options:
| |
| |	OperationID: signIn
| |	Arguments: [username, password]
| |	ArgumentsInQuerystring: False
| |	HideInAPI: False
| |	MinimumAuthenticationLevelToCall: None
| |	ParseAsJson: True
| |
\*/

using System;
using System.Collections;

namespace ModBotWebsiteAPI
{
    public static partial class API
    {
        public static void SignIn(string username, string password, Action<JsonObject> callback)
        {
            StaticCoroutineRunner.StartStaticCoroutine(_signIn(username, password, callback));
        }

        private static IEnumerator _signIn(string username, string password, Action<JsonObject> callback)
        {

            string url = MODBOT_API_URL_BASE;
            string data = "";

            url += "signIn";
            JsonConstructor json = new JsonConstructor();
            json.AppendValue("username", username);
            json.AppendValue("password", password);
            data = json.ToString();

            yield return SendRequest(url, data, callback);
        }
    }
}
